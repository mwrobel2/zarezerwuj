import { Component, OnInit, OnDestroy } from '@angular/core';
import { Przesylki } from '../../models/przesylki.model';
import { PrzesylkiService } from '../../services/przesylki.service';
import { Przesylkiform } from '../../models/formatki/przesylkiform.model';
import { Subscription, from } from 'rxjs';
import { User } from '../../models/user.model';
import { PageEvent } from '@angular/material/paginator';
import { NgForm } from '@angular/forms';
import { Router } from '@angular/router';
import { LogowanierejestracjaService } from '../../services/logowanierejestracja.service';
import { environment } from '../../../environments/environment';
import { PlikiService } from '../../services/pliki.service';

@Component({
  selector: 'app-przesylki',
  templateUrl: './przesylki.component.html',
  styleUrls: ['./przesylki.component.scss']
})
export class PrzesylkiComponent implements OnInit, OnDestroy {

  przesylki: Przesylki[] = [];
  private przesylkiSub: Subscription;
  public zalogowanyUser: Subscription;
  userIsAuthenticated = false;

  public loggedInUser: User = {
    id: null,
    _id: null,
    login: null,
    email: null,
    department: null,
    name: null,
    surname: null,
    moduly: null,
    contractorFields: null,
    szkodliwaFields: null,
    assortmentFields: null
  };

  totalPrzesylki = 0;
  przesylkiPerPage = 10;
  currentPage = 1;
  pageSizeOptions = [5, 10, 20, 50];
  plikiKolumny: string[];

  private searchValues = {
    fullName: '',
    comments: '',
    accountManagerLogin: '',
    nrZapotrzebowania: '',
    doKogo: '',
    status: ''
  };

  przesylka = {
    fullName: '',
    comments: '',
    nrZapotrzebowania: '',
    doKogo: '',
    rodzajPlatnosci: '',
    kwota: null,
    status: ''
  };

  isLoading = false;

   // I declare displayed fields
   dispFields: Przesylkiform = {
    comments: false,
    status: false,
    buttonDodajKontr: false,
    buttonEdytujKontr: false,
    buttonUsunKontr: false,
    widziWszystkie: false,
    firmy: false,
    projekty: false,
    pliki: false,
    buttonPliki: false
  };
  backendUrl = environment.backendUrl;


  constructor(
    private logowanierejestracjaService: LogowanierejestracjaService,
    public przesylkiService: PrzesylkiService,
    private router: Router,
    private plikiService: PlikiService
  ) { }

  ngOnInit(): void {
    this.isLoading = true;
    this.zalogowanyUser = this.logowanierejestracjaService.getUser().subscribe(
      user => {
        this.loggedInUser = user;
        if (typeof this.loggedInUser !== 'undefined' && this.loggedInUser.moduly !== null
          && typeof this.loggedInUser.moduly !== 'undefined') {
          this.userIsAuthenticated = true;
          this.dispFields = this.loggedInUser.przesylkiFields;

          if (!this.dispFields.widziWszystkie) {
            this.searchValues.accountManagerLogin = this.loggedInUser.login;
          }
          // po zweryfikowaniu usera pobieram pliki z bazy
          this.przesylkiService.getPrzesylki(
            this.przesylkiPerPage,
            this.currentPage,
            this.searchValues
          );
        }
      },
      error => {
        this.router.navigate(['/login']);
      });

    this.przesylkiSub = this.przesylkiService
      .getPrzesylkiUpdatedListener()
      .subscribe(
        (przesylkiData: {
          przesylki: Przesylki[];
          przesylkiCount: number;
        }) => {
          this.isLoading = false;
          this.totalPrzesylki = przesylkiData.przesylkiCount;
          this.przesylki = przesylkiData.przesylki;
        }
      );

    this.plikiKolumny = [
      'name',
      'rodzaj',
    ];
  }

  powrot() {
    window.history.go(-1);
  }

  onDelete(przesylkaId: string, pliki: any) {
    if (confirm('Czy na pewno usunąć informację o tej przesyłce?')) {
      let liczbaPlikow = pliki.length;

      if (pliki.length > 0) {
        pliki.forEach((plik: { fileName: string, katalog: string }) => {
          // console.log('Usuwany plik', plik.fileName);
          this.plikiService.ksujPlik(plik.fileName, plik.katalog).subscribe((res: { usuniety: string }) => {
            liczbaPlikow--;
            //  nawet jeżeli z jakichś powodów pliku nie dało się usunąć
            // if (res.usuniety === 'ok' && liczbaPlikow <= 0) {
            if (liczbaPlikow <= 0) {
              this.isLoading = true;
              this.przesylkiService.deletePrzesylki(przesylkaId).subscribe(() => {
                this.przesylkiService.getPrzesylki(
                  this.przesylkiPerPage,
                  this.currentPage,
                  this.searchValues
                );
              });
            }
          });
        });
      } else {
        // nie było plików do usunięcia więc usuwam wpis z bazy
        this.isLoading = true;
        this.przesylkiService.deletePrzesylki(przesylkaId).subscribe(() => {
          this.przesylkiService.getPrzesylki(
            this.przesylkiPerPage,
            this.currentPage,
            this.searchValues
          );
        });
      }
    }
  }

  logout() {
    if (confirm('Czy na pewno chcesz się wylogować z systemu?')) {
      this.logowanierejestracjaService.logout().subscribe(
        data => { this.router.navigate(['/']); },
        error => console.error(error)
      );
    }
  }

  onChangedPage(pageData: PageEvent) {
    this.isLoading = true;
    this.currentPage = pageData.pageIndex + 1;
    this.przesylkiPerPage = pageData.pageSize;
    this.przesylkiService.getPrzesylki(
      this.przesylkiPerPage,
      this.currentPage,
      this.searchValues
    );
  }

  onSearchPrzesylki(form: NgForm) {
    if (form.invalid) {
      return;
    }
    this.searchValues = {
      fullName: form.value.nazwaPelna,
      comments: form.value.comments,
      accountManagerLogin: null,
      doKogo: form.value.doKogo,
      nrZapotrzebowania: form.value.nrZapotrzebowania,
      status: form.value.status
    };
    if (!this.dispFields.widziWszystkie) {
      this.searchValues.accountManagerLogin = this.loggedInUser.login;
    }
    this.currentPage = 1;
    this.przesylkiService.getPrzesylki(
      this.przesylkiPerPage,
      this.currentPage,
      this.searchValues
    );
    this.przesylkiSub = this.przesylkiService
      .getPrzesylkiUpdatedListener()
      .subscribe(
        (przesylkiData: {
          przesylki: Przesylki[];
          przesylkiCount: number;
        }) => {
          // DO ZASTANOWIENIA CZY PRZEKAZYWAĆ WSZYSTKICH KONTRAKTORÓW W TEN SPOSÓB ....
          this.isLoading = false;
          this.totalPrzesylki = przesylkiData.przesylkiCount;
          this.przesylki = przesylkiData.przesylki;
          // console.log(this.assortments);
          this.przesylkiSub.unsubscribe();
        }
      );
  }

  clearForm(form: NgForm) {
    this.przesylka.fullName = '';
    this.przesylka.comments = '';
    this.przesylka.nrZapotrzebowania = '';
    this.przesylka.doKogo = '';
    this.przesylka.rodzajPlatnosci = '';
    this.przesylka.kwota = null;
    this.przesylka.status = '';
  }

  ngOnDestroy(): void {
    this.przesylkiSub.unsubscribe();
    this.zalogowanyUser.unsubscribe();
  }

}

