...
import { Dictionary } from "../models/slownik2.model";
// 'event emitter'
...
export class DictionaryService {
  ...
  getDictionaries() {
    ...
    this.http
      .get<{ message: string; dictionaries: any }>(
        environment.apiUrl + "/dictionary2",
  ...
  getDictionary(id: string) {
    ...
    return this.http.get<{ _id: string }>(
      environment.apiUrl + "/dictionary2/" + id,
      { withCredentials: true }
    );
  }
  ...
  getDictionaryName(name: string) {
    return this.http.get<{ _id: string; values: [] }>(
      environment.apiUrl + "/dictionary2/name/" + name,
      { withCredentials: true }
    );
  }
  ...
  updateDictionary(id: string, dictionaryData: Dictionary) {
    ...
    this.http
      .put(environment.apiUrl + "/dictionary2/" + id, dictionary, {
        withCredentials: true
      })
      .subscribe(response => {
  ...
  deleteDictionary(dictionaryId: string) {
    this.http
      .delete(environment.apiUrl + "/dictionary2/" + dictionaryId, {
        withCredentials: true
      })
      .subscribe(() => {
